...
#!/bin/sh
if [ "${1:-}" = "--version" ]; then echo "skillwick 0.1.0"; exit 0; fi
exit 0
EOF
...
HOME="$home" SKILLWICK_BASE_URL="file://$server" sh "$root/scripts/install.sh" --version 0.1.0 --prefix "$prefix"
test "$("$prefix/bin/skillwick" --version)" = "skillwick 0.1.0"
...
if [ "$#" -eq 1 ]; then
  ...
  HOME="$home" SKILLWICK_BASE_URL="file://$1" sh "$root/scripts/install.sh" --version 0.1.0 --prefix "$real_prefix"
  test "$("$real_prefix/bin/skillwick" --version)" = "skillwick 0.1.0"
fi
