...
Record the one snapshot marker that owns the published native corpus: executable/version, Codex home, and cwd scope. Native enablement is authoritative for native records; never resurrect a native-disabled record through the filesystem adapter. Outside the snapshot workspace, retain only portable user/system/global native records and get project skills from the current filesystem scan. A different Codex home requires refresh. Do not load an old cached plugin version merely because its directory still exists.
...
Ordinary `.agents/skills` changes are detected by local scanning. A newly added native plugin with a new root becomes searchable after native refresh; the workstation pipeline must call it after installs. Out-of-band plugin installs or policy changes therefore have a documented one-command recovery: `skillwick refresh`. `read` enforces the published enablement state and revalidates the active path and content hash without launching nested Codex. The explicit refresh boundary owns native policy freshness.
